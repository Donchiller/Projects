...
const Creatine = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3000/creatineProducts") // Adjust endpoint as needed
      .then((res) => {
        const cleanData = res.data.filter(
          (item) =>
            item.name && item.price && item.category && item.brand && item.image
        );
        setCreatineProducts(cleanData);
      })
      .catch((err) => console.error("Error fetching creatine products:", err));
  }, []);

  // Filter products based on selected filters
  const filteredProducts = creatineProducts.filter((product) => {
    ...
    <div className="container py-5" style={{ backgroundColor: "#f5f5f5" }}>
      <div className="row">
        ...
        <div className="col-md-9">
          <h3 className="fw-bold mb-4 text-center">Creatine Products</h3>
          <div className="row g-4">
            {filteredProducts.map((product) => (
              <div className="col-6 col-md-4" key={product.id}>
                <Link
                  style={{ textDecoration: "none" }}
                  to={"/Creatinedynamic/" + product.id} // Adjust the route as needed
                >
