...
const Preworkout = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3000/preworkoutProducts")
      .then((res) => {
      ...
      .catch((err) =>
        console.error("Error fetching pre-workout products:", err)
      );
  }, []);
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const matchesDiscount =
      ...
      selectedDiscounts.some(
        (discount) => product.discount && product.discount >= parseInt(discount)
      );
    const matchesPrice =
    ...
    <div className="container py-5" style={{ backgroundColor: "#f5f5f5" }}>
      <div className="row">
        ...
        <div className="col-md-9">
          <h3 className="fw-bold mb-4 text-center">Pre-Workout Products</h3>
          <div className="row g-4">
            {filteredProducts.map((product) => (
              <div className="col-6 col-md-4" key={product.id}>
                <Link
                  to={`/PreworkoutDynamic/${product.id}`}
