...
const weights = ["500 g (1.1 lb)", "1 kg (2.2 lb)", "2 kg (4.4 lb)"];

const Plantproteindynamics = () => {
  const { id } = useParams();
  console.log("Plant Protein Id", id);
  ...
  const fetchDataById = async () => {
    try {
      const resp = await axios.get(
        `http://localhost:3000/plantProteinProducts/${id}`
      );
...
export default Plantproteindynamics;
